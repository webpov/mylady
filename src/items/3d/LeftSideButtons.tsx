...
const ButtonComponent = ({ onClick, isActive, icon, iconTransform = "", iconScale = "" }:any) => {
  ...
  return (
    <button onClick={onClick} style={style} className={`flex-col h-min-50px  tx-center w-100 box-shadow-5-b noborder bord-r- px-2 opaci-chov--50 tx-lx pt-1 ${activeClass} bord-r-15 bg-glass-10 bg-w-10`}>
...
export const LeftSideButtons = ({ toggleOption, optsToggler, zOut, xOut }:any) => {
  ...
  return (
    <>
      <div className='flex flex-justify-center my-4 gap-4 pos-fix w-100 z-800  ' >
                    
        <div className="pos-abs top-0 left-0 flex gap-2 tx-white  z-800  ">
          <Link href="/" className="a tx-xl flex z-400 tx-white tx-altfont-3 ">
            <img src="/img/mylady.png" width={64} alt="logo" className="" style={{
              filter: "hue-rotate(-119deg) brightness(100%) invert(1) "+(colorIndex != "dd9900" ? "saturate(0)" : ""),
            }} />
            ...
            <div className="a tx-xl tx-bold-2 tx-altfont-1" style={{color:"#"+colorIndex}}>Lady</div>
            {/* <img src="/img/mylady.png" width={64} alt="logo" className="px-1" /> */}
          </Link>
          {/* <div className="a tx-xl">Duno</div> */}
        </div>
        <div className=' tx-blue  tx-lg px-8 opaci-chov--50 z-800 pos-abs right-0'
          style={{filter: "hue-rotate(-189deg) brightness(666%)", }}
        >
          <HiCubeTransparent />
        </div>
        <div className='tx-white blur-4 z-500 pos-abs top-0 right-0 noclick'>
          <Link href="/config/global/" className=' tx-blue tx-lg px-8 opaci-chov--50 '
            style={{filter: "hue-rotate(-189deg) brightness(666%)", }}
          >
            <HiCubeTransparent />
          </Link>
      ...
      <div className="flex pos-abs top-0 left-0 bord-r- pa-1 ma-1 mt-4">
